...
| `BIZSCOUT_MAX` | `30` | **保存検索1件あたり**の取り込み上限（検索スカウト用）。対象条件を満たさない候補者が大半なので、送信したい件数より十分大きくしておく。送信合計は `BIZSCOUT_MAX_SENDS_PER_RUN` が上限で、そこに達した時点で取り込みも打ち切るため、この値を大きくしても送信数が増えることはない |
...
| `BIZSCOUT_MAX_SENDS_PER_RUN` | `10` | **1回の実行で送る初回スカウトの合計上限**。保存検索ごとではなく**全保存検索の合計**（4つ登録していても合計でこの件数まで）。この件数に達した時点で残りの保存検索は開かず、次回の実行に回す。再送にも同じ値が別枠で適用される。ピックアップは無料枠のため適用されない |
