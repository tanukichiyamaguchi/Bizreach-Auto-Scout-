...
| `BIZSCOUT_MAX` | `30` | **保存検索1件あたり**の最大取り込み件数（検索スカウト用）。保存検索を4つ登録していれば最大 4×30 名を評価する（実際に送る数は `BIZSCOUT_MAX_SENDS_PER_RUN` が上限） |
| `BIZSCOUT_REEVALUATE_AFTER_DAYS` | `30` | 直近この日数以内に評価済みの候補者は取り込みで読み飛ばし、検索結果の次ページへ進む（任意・未設定なら30）。**0 にすると毎回同じ先頭N件を取り直す**ため、通常は変更しない |
