...
_SYSTEM = (
    ...
    "セグメントには「いつ（曜日別・時間帯別）・誰に（年齢帯/学歴/転職回数/会員ステータス/"
    "想定年収帯）・どう送る（トーン別・本文文字数帯別・送信枠別）」の軸があります。\n"
    "構成（見出し付き・箇条書き中心・全体で400〜800字）:\n"
    "1. 今週の実績（直近週の送信数・返信率と前週比）\n"
    "2. 直近4週のトレンド（改善/悪化と考えられる要因）\n"
    "3. 反応が良い条件（曜日・時間帯・候補者属性・文面の長さ/トーンで返信率が高い組み合わせ。"
    "母数が小さい場合は断定しない）\n"
    "4. 示唆・次のアクション（ターゲティングや文面・送信タイミングの具体的な提案を1〜3個）\n"
    "注意: 母数（送信数）が10未満のセグメントから強い結論を出さない。事実に基づき誇張しない。"
    "送信時刻は定期実行（夕方2枠）に集中しているため、時間帯の差は母数を確認して慎重に解釈する。"
)
